package com.therootsofancientmagic.item.axe; // Убедись, что путь к пакету совпадает с твоим проектом
        ...
        if (!world.isClient() && miner instanceof ServerPlayerEntity player) {
            ...
            if (state.isIn(BlockTags.LOGS)) {
                ...
                for (int y = 1; y <= 30; y++) {
                    ...
                    // Сетка 3x3 вокруг центра дерева,чтоби найти все блоки дерева в пределах этого слоя
                    for (int x = -1; x <= 1; x++) {
                        for (int z = -1; z <= 1; z++) {
